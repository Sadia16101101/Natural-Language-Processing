...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    val = np.matmul(outsideVectors, centerWordVec)
    soft_val = softmax(val)

    loss = -np.log(soft_val[outsideWordIdx])

    val1 = soft_val.copy()
    val1[outsideWordIdx] = val1[outsideWordIdx] - 1
    gradCenterVec = np.matmul(outsideVectors.T, val1)

    gradOutsideVecs = np.outer(val1, centerWordVec)
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    loss = loss - np.log(z)
    gradCenterVec = gradCenterVec + u_o*(z-1)
    ...
    for i in range(K):
        ...
        loss = loss - np.log(z)
        gradCenterVec = gradCenterVec + u_k*(1-z)
        gradOutsideVecs[neg_id] =  gradOutsideVecs[neg_id] + centerWordVec*(1-z)
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        ...
        loss1, gradCenter1, gradOutside1= word2vecLossAndGradient(centerWordVec=centerWordVec, outsideWordIdx=outside_id,outsideVectors=outsideVectors,dataset=dataset)
        loss = loss + loss1

        gradCenterVecs[center_id] = gradCenterVecs[center_id] + gradCenter1
        gradOutsideVectors = gradOutsideVectors + gradOutside1
    ### END YOUR CODE
